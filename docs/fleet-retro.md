...
| Deploys | *(not yet a dedicated source)* | Surfaces indirectly today via feed entries whose title/body mention a deploy, and via PR merges. A provider-neutral deploy-log collector is a natural follow-up but was out of scope for this pass; noted here rather than silently claimed as covered. |
